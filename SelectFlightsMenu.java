...
public class SelectFlightsMenu extends JDialog implements ActionListener {
	...
	private final int ICON_COLUMN = 1;
	...
	private final int ACIDS_COLUMN = 6;
	 ...
	 */
	public SelectFlightsMenu(JFrame parent, List<FlightFilter> filtersIn) {
	...
	private void addEntry(FlightFilter f) {
		...
		tableModel.addRow(new Object[] { temp.isShow(), temp.getIcon(), temp.getName(), temp.getDest(), temp.getdFix(),
				new Object[] { f, temp }, temp.getAcidsAsString() });
	}
		...
		if (e.getActionCommand().equals("Apply") || e.getActionCommand().equals("OK")) {
			...
			for (int row = 0; row < tableModel.getRowCount(); row++) {
				...
				if (index > -1) {
					...
					f = tableModel.getTempFlight(row);
					filters.add(row, f);
					tableModel.setRealFlight(row, f);
				}
